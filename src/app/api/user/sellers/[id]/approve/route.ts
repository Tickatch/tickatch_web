...
/**
 * POST /api/user/sellers/[id]/approve - 판매자 승인 (PENDING → APPROVED)
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/sellers/${id}/approve`, {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json<ApiResponse<null>>(
          { success: false, error: data.error || { code: "APPROVE_ERROR", message: "판매자 승인에 실패했습니다.", status: response.status } },
