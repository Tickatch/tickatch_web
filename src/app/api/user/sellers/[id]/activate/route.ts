...
/**
 * POST /api/user/sellers/[id]/activate - 판매자 활성화 (정지 해제)
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/sellers/${id}/activate`, {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json<ApiResponse<null>>(
          { success: false, error: data.error || { code: "ACTIVATE_ERROR", message: "판매자 활성화에 실패했습니다.", status: response.status } },
